...
const base_URL = `https://locations.bk.com/search.html`//`https://maps.google.com`;//`https://www.google.com/maps/@${lat_init},${lon_init},3a,75y,182.14h,98.61t/data=!3m6!1e1!3m4!1sqL97FDTbhSXGPhQC_v_-xQ!2e0!7i16384!8i8192`;
const completed_file = 'bk/completed.txt';
const all_zips_file = 'US_zips.txt';
const err_file = 'bk/error.txt';
...
(async () => {
  ...
  while (true) {
    ...
    for (var j = 0; j<zips_batch.length; j++) {
      ...
      await page.waitForSelector("#js-filters-dropdown");
      // const found = await page.evaluate(() => window.find("there are no locations"));
      let innerHTML = await page.$eval('div.location-list-results', elt => elt.innerHTML);
    ...
    save_file = 'bk/'+Date.now().toString()+'.JSON';
